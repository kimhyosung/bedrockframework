package com.bedrockframework.plugin.util
{
	...
	public class XMLUtil extends StaticWidget
	{
		...
		public static function getIDValueObject( $node:* ):Object
		{
			var objResult:Object = new Object();
			objResult[ $node.@id ] = VariableUtil.sanitize( $node.@value );
			return objResult;
		}
		
		public static function convertToXML( $data:* ):XML
		{
			if ( $data is XMLList ) {
		...
		public static function filterByAttribute( $data:*, $attribute:String, $value:String):XML
		{
			var xmlData:XML = XMLUtil.convertToXML( $data ).children().(attribute($attribute) == $value )[ 0 ];
			return xmlData;
		}
		public static function filterByNode( $data:*, $name:String, $value:String):XML
		{
			var xmlList:XMLList = XMLUtil.convertToXML( $data ).children().(child($name) == $value);
			return XMLUtil.convertToXML( xmlList );
		}

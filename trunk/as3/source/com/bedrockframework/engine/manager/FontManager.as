package com.bedrockframework.engine.manager
{
	...
	public class FontManager extends StandardWidget implements IFontManager
	{
		...
		{
			if (FontManager.__bolInitialized) return;
			
			var objByteArray:SWFByteArray = new SWFByteArray();
			objByteArray.writeBytesFromString(
			...
			objByteArray.readBytes(FontManager.SWF_HEADER);
			objByteArray.length = 0;
			objByteArray.writeBytesFromString(
			...
			objByteArray.position = 0;
			objByteArray.readBytes(FontManager.CLASS_CODE);
			objByteArray.length = 0;
			
			FontManager.__bolInitialized = true;
		}
		
		
		private var _objBackgroundLoader:BackgroundLoader;
		private var _objBytes:ByteArray;
		private var _numFontCount:uint;
		private var _bolAutoRegister:Boolean;
		private var _arrFonts:Array;
		private var _objLibraryLoader:Loader;
		...
		*/
		...
		{
			FontManager.setup();
			this._bolAutoRegister = true;
			this._arrFonts = new Array;
		...
		private function createBackgroundLoader():void
		{
			...
			this._objBackgroundLoader.dataFormat = URLLoaderDataFormat.BINARY;
			this._objBackgroundLoader.addEventListener(LoaderEvent.COMPLETE, this.onSWFLoadComplete);
			this._objBackgroundLoader.addEventListener(LoaderEvent.IO_ERROR, this.onSWFLoadError);
			this._objBackgroundLoader.addEventListener(LoaderEvent.SECURITY_ERROR, this.onSWFLoadError);
		}
		 ...
		 */
		public function load($url:String, $autoRegister:Boolean = true):void
		{
			this.close();
			this.clear();
			this._bolAutoRegister = $autoRegister;
			this._objBackgroundLoader.loadURL( $url );
		}
		...
		public function close():void {
			try {
				this._objBackgroundLoader.close();
			} catch (error:Error) {}
			
			if (this._objLibraryLoader) {
				this._objLibraryLoader.contentLoaderInfo.removeEventListener(Event.COMPLETE, this.onLibraryLoadComplete);
				
				try {
					this._objLibraryLoader.close();
				} catch (error:Error) {}

				try {
					this._objLibraryLoader.unload();
				} catch (error:Error) {}

				this._objLibraryLoader = null;
			}
		}
		...
		public function registerFonts():void {
			for each (var font:Font in this._arrFonts) {
		...
		private function analyze($bytes:ByteArray, $delayed:Boolean = false):void 
		{
			...
			try {
				...
				if ($delayed) {
					this._objLibraryLoader = new Loader();
					this._objLibraryLoader.addEventListener(Event.ENTER_FRAME, this.onDelayedVerifyError);
				} else {
					this.close();
			...
			var classCodeLength:uint = FontManager.CLASS_CODE.length;
			...
			tempData = new ByteArray();
			tempData.endian = Endian.LITTLE_ENDIAN;
			tempData.writeBytes(FontManager.SWF_HEADER);
			id = 0;
			...
			for (o in fontData) {
				...
				if (data) {
					...
					stringID = FontManager.CLASS_NAME_PREFIX + stringID;
					tempData.writeShort(FontManager.TAG_DO_ABC);
					...
					tempData.writeBytes(FontManager.CLASS_CODE);
					tempData.writeBytes(data);
					tempData.writeShort(FontManager.TAG_SYMBOL_CLASS);
			...
			this._numFontCount = id;
			
			if (this._numFontCount) {
				...
				this._objLibraryLoader = new Loader();
				this._objLibraryLoader.contentLoaderInfo.addEventListener(Event.COMPLETE, this.onLibraryLoadComplete);
				...
				this._objLibraryLoader.loadBytes(fontSWF, context);
			} else {
				if ($delayed) {
					this._objLibraryLoader = new Loader();
					this._objLibraryLoader.addEventListener(Event.ENTER_FRAME, this.onDelayedComplete);
				} else {
					this.close();
				}
			}			
		...
		private function onDelayedComplete($event:Event):void 
		{
			this._objLibraryLoader.removeEventListener(Event.ENTER_FRAME, this.onDelayedComplete);
			this._objLibraryLoader = null;
			this.close();
		}
		
		private function onDelayedVerifyError($event:Event):void 
		{
			this._objLibraryLoader.removeEventListener(Event.ENTER_FRAME, this.onDelayedVerifyError);
			this._objLibraryLoader = null;
			this.close();
		}
		...
		{
			this.analyze(this._objBackgroundLoader.data as ByteArray);
			this.status("Fonts Loaded");
		}
		private function onSWFLoadError($event:Event ):void
		{
			this.warning("Failed to load font swf!");
		}
		...
		private function onLibraryLoadComplete($event:Event):void
		{
			...
			var fontClass:Class;
			var font:Font;
			var domain:ApplicationDomain = this._objLibraryLoader.contentLoaderInfo.applicationDomain;
			
			for (i = 0;i < this._numFontCount;i++) {
				id = i.toString();
				while (id.length < 3) id = '0' + id;
				id = FontManager.CLASS_NAME_PREFIX + id;
				...
				if (domain.hasDefinition(id)) {
					...
					if (font && font.fontName) { // Skip static fonts
						this._arrFonts.push(font);
						if (this._bolAutoRegister) Font.registerFont(fontClass);
					}
				}
			}
		...
		*/
		...
		{
			if (this._bolAutoRegister == value) return;
			this._bolAutoRegister = value;
			if (value) this.registerFonts();
		}
		
		public function get autoRegister():Boolean
		{
			return this._bolAutoRegister;
		}
		
		public function get bytesLoaded():uint
		{
			return this._objBackgroundLoader.bytesLoaded;
		}
		
		public function get bytesTotal():uint 
		{
			return this._objBackgroundLoader.bytesTotal;
		}
		/**

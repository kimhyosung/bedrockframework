package com.bedrock.framework.plugin.view
{
	...
	public class ViewStack extends SpriteBase
	{
		...
		*/
		...
		private var _objSequence:ViewSequence;
		...
		*/
		...
		public function initialize($data:ViewStackData):void
		{
			...
			if ( this.data.startingAlias != null ) {
				var numIndex:int = CoreUtil.findIndex( this.data.stack, this.data.startingAlias, "id" );
				if ( numIndex != -1 ) {
		...
		*/
		...
		public function selectByAlias( $id:String ):void
		{
			var numIndex:int = CoreUtil.findIndex( this.data.stack, $id, "id" );
			if ( numIndex != -1 ) {
				this.selectByIndex( numIndex );
			} else {
				this.error( "Alias '" + $id + "' not found!" );
			}
		}
		/*
		Get View
		*/
		...
		{
			return this._arrViews.findItem( $id, "id" );
		}
		/*
		...
		*/
		...
		{
			this.call( ViewStack.INTRO );
			this.dispatchEvent( new ViewEvent( ViewEvent.INITIALIZE_COMPLETE, this, this.getDetailObject() ) );
		}
		private  function onIntroComplete($event:ViewEvent):void
		{
			if (this.data.timerEnabled) this.startTimer();	
			this.dispatchEvent( new ViewEvent( ViewEvent.INTRO_COMPLETE, this, this.getDetailObject() ) );
		}
		private function onOutroComplete($event:ViewEvent):void
		{
			this.dequeue();
			this.dispatchEvent( new ViewEvent( ViewEvent.OUTRO_COMPLETE, this, this.getDetailObject() ) );
		}

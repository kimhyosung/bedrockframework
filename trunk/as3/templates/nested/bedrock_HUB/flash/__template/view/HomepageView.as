	...
	public class HomepageView extends BedrockView implements IView
	{
		...
		*/
		...
		private function createCloner():void
		{
			...
			this._objCloner = new Cloner( VisualLoader );
		...
		*/
		...
		private function onLoadComplete( $event:LoaderEvent ):void
		{
			...
			clsBedrockDispatcher.addEventListener( BedrockEvent.LOAD_COMPLETE, this.onFuckinA );
			trace( clsBedrockDispatcher );
		}
		
		private function onFuckinA( $event:* ):void
		{
			trace( "Sack" );
			debug( $event.details );
		}

package com.bedrock.extras.pagination
{
	...
	public class Pagination extends DispatcherBase implements IPageable
	{
		...
		*/
		public function update($total:uint = 0, $itemsPerPage:uint = 0):void
		{
			...
			Bedrock.logger.status("Total Items - " + this._totalItems);
			Bedrock.logger.status("Total Pages - " + this._totalPages);
			this._checkBounds();
		...
		*/
		public function selectPage($index:uint):uint
		{
			if ( $index >= this._totalPages ) {
				...
				Bedrock.logger.status("Selected Page - " + this._selectedPage);

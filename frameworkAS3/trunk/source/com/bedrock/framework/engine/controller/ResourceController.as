	...
	public class ResourceController extends StandardBase implements IResourceController
	{
		...
		*/
		...
		public function queue( $autoLoad:Boolean = true ):void
		{
			...
			this._queueSharedAssets( BedrockEngine.config.getSettingValue( BedrockData.LIBRARY_ENABLED ), BedrockEngine.localeManager.isFileLocalized( BedrockData.LIBRARY ), currentLocale );
		...
		private function _queueSharedAssets( $enabled:Boolean, $localized:Boolean, $locale:String = null ):void
		{
			if ( $enabled ) {
				...
				loader.vars.context = this._getLoaderContext();
				if ( loader != null ) {
					loader.addEventListener( LoaderEvent.COMPLETE, this._onSharedAssetsComplete, false, 10000 );
					BedrockEngine.loadController.appendLoader( loader );
				} else {
					this.warning( "SharedAssets loader does not extend LoaderItem!" );
				}
			}
		}
		...
		private function _onSharedAssetsComplete( $event:LoaderEvent ):void
		{
